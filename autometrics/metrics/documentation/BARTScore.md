...
$$
BARTScore = \sum _{t=1}^{m} \omega_t \log p(y_t | y_{<t}, x, \theta)
$$

where:

- $p(y_t | y_{<t}, x, \theta)$ is the probability of the $t$-th token in the hypothesis $y$ given the preceding tokens and the source/reference text $x$ under the BART model parameters $\theta$.
- $\omega_t$ is an optional weighting factor (default: uniform).
