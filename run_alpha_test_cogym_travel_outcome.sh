...
#SBATCH --job-name=alpha_test_cogym_travel_outcome
#SBATCH --account=nlp
...
#SBATCH --output=alpha_test_cogym_travel_outcome.out
#SBATCH --error=alpha_test_cogym_travel_outcome.err

# =============================================================================
# SLURM Job Script for Alpha Comparison Test - CoGymTravelOutcome Dataset
# =============================================================================
...
log_info "Starting alpha comparison test for CoGymTravelOutcome dataset"
...
# Check if CoGym dataset file exists
if [ ! -f "autometrics/dataset/datasets/cogym/cogym.csv" ]; then
    log_error "CoGym dataset file not found: autometrics/dataset/datasets/cogym/cogym.csv"
    exit 1
fi
...
# =============================================================================
# Run the Alpha Comparison Test for CoGymTravelOutcome
# =============================================================================

log_info "Launching alpha comparison test for CoGymTravelOutcome dataset..."

# Set environment variables
export PYTHONPATH="${PYTHONPATH:-}:$(pwd)"

# Set dataset-specific DSPY cache directory
export DSPY_CACHEDIR="/nlp/scr3/nlp/20questions/dspy_cache/autometrics_alpha_test_cogym_travel_outcome"
...
log_info "  - Dataset: CoGymTravelOutcome"
log_info "  - Target Measure: outcomeRating (auto-detected)"
log_info "  - Seed: $SEED"
log_info "  - Regenerate Metrics: $REGENERATE"
log_info "  - Generated Metrics Dir: generated_metrics_CoGymTravelOutcome"
log_info "  - DSPY Cache Dir: $DSPY_CACHEDIR"

# Build command
CMD="python test_regression_alpha_comparison.py --dataset CoGymTravelOutcome --target-measure outcomeRating --seed $SEED"
...
# Run the test script
if eval $CMD; then
    log_success "Alpha comparison test for CoGymTravelOutcome completed successfully!"
    
    # Show output file size
    if [ -f "alpha_test_cogym_travel_outcome.out" ]; then
        log_info "Output file size: $(du -h alpha_test_cogym_travel_outcome.out | cut -f1)"
    fi
    
    exit 0
else
    log_error "Alpha comparison test for CoGymTravelOutcome failed!"
    
    # Show recent error output
    if [ -f "alpha_test_cogym_travel_outcome.err" ]; then
        log_info "Error output (last 20 lines):"
        tail -n 20 alpha_test_cogym_travel_outcome.err
    fi
