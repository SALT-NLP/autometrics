...
#SBATCH --job-name=alpha_test_evalgen
#SBATCH --account=nlp
...
#SBATCH --output=alpha_test_evalgen.out
#SBATCH --error=alpha_test_evalgen.err

# =============================================================================
# SLURM Job Script for Alpha Comparison Test - EvalGenProduct Dataset
# =============================================================================
...
log_info "Starting alpha comparison test for EvalGenProduct dataset"
...
# =============================================================================
# Run the Alpha Comparison Test for EvalGenProduct
# =============================================================================

log_info "Launching alpha comparison test for EvalGenProduct dataset..."

# Set environment variables
export PYTHONPATH="${PYTHONPATH:-}:$(pwd)"

# Set dataset-specific DSPY cache directory
export DSPY_CACHEDIR="/nlp/scr3/nlp/20questions/dspy_cache/autometrics_alpha_test_evalgen"
...
log_info "  - Dataset: EvalGenProduct"
log_info "  - Target Measure: grade (auto-detected)"
log_info "  - Seed: $SEED"
log_info "  - Regenerate Metrics: $REGENERATE"
log_info "  - Generated Metrics Dir: generated_metrics_EvalGenProduct"
log_info "  - DSPY Cache Dir: $DSPY_CACHEDIR"

# Build command
CMD="python test_regression_alpha_comparison.py --dataset EvalGenProduct --target-measure grade --seed $SEED"
...
# Run the test script
if eval $CMD; then
    log_success "Alpha comparison test for EvalGenProduct completed successfully!"
    
    # Show output file size
    if [ -f "alpha_test_evalgen.out" ]; then
        log_info "Output file size: $(du -h alpha_test_evalgen.out | cut -f1)"
    fi
    
    exit 0
else
    log_error "Alpha comparison test for EvalGenProduct failed!"
    
    # Show recent error output
    if [ -f "alpha_test_evalgen.err" ]; then
        log_info "Error output (last 20 lines):"
        tail -n 20 alpha_test_evalgen.err
    fi
