...
#SBATCH --job-name=alpha_test_helpsteer
#SBATCH --account=nlp
...
#SBATCH --output=alpha_test_helpsteer.out
#SBATCH --error=alpha_test_helpsteer.err
#SBATCH --requeue
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=mryan0@stanford.edu

# =============================================================================
# SLURM Job Script for Alpha Comparison Test - HelpSteer Dataset
# =============================================================================
...
log_info "Starting alpha comparison test for HelpSteer dataset"
...
# =============================================================================
# Run the Alpha Comparison Test for HelpSteer
# =============================================================================

log_info "Launching alpha comparison test for HelpSteer dataset..."

# Set environment variables
export PYTHONPATH="${PYTHONPATH:-}:$(pwd)"

# Set dataset-specific DSPY cache directory
export DSPY_CACHEDIR="/nlp/scr3/nlp/20questions/dspy_cache/autometrics_alpha_test_helpsteer"
export AUTOMETRICS_CACHE_DIR="/nlp/scr3/nlp/20questions/autometrics_cache_helpsteer"
...
log_info "  - Dataset: HelpSteer"
log_info "  - Target Measure: helpfulness (auto-detected)"
log_info "  - Seed: $SEED"
log_info "  - Regenerate Metrics: $REGENERATE"
log_info "  - Generated Metrics Dir: generated_metrics_HelpSteer"
log_info "  - DSPY Cache Dir: $DSPY_CACHEDIR"

# Build command
CMD="python test_regression_alpha_comparison.py --dataset HelpSteer --target-measure helpfulness --seed $SEED"
...
# Run the test script
if eval $CMD; then
    log_success "Alpha comparison test for HelpSteer completed successfully!"
    
    # Show output file size
    if [ -f "alpha_test_helpsteer.out" ]; then
        log_info "Output file size: $(du -h alpha_test_helpsteer.out | cut -f1)"
    fi
    
    exit 0
else
    log_error "Alpha comparison test for HelpSteer failed!"
    
    # Show recent error output
    if [ -f "alpha_test_helpsteer.err" ]; then
        log_info "Error output (last 20 lines):"
        tail -n 20 alpha_test_helpsteer.err
    fi
